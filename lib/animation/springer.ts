 ...
 * Extracted and optimized from reference animation architecture.
 */
...
export function createSpring(tension = 0.5, friction = 0.5): (t: number) => number {
  ...
  return (t: number): number => {
    return points[Math.ceil(t * (points.length - 1))] ?? 1
  }
}

export const Springer = {
  default: (tension = 0.2, friction = 0.8) => createSpring(tension, friction),
  snappy: (tension = 0.4, friction = 0.6) => createSpring(tension, friction),
  gentle: (tension = 0.15, friction = 0.85) => createSpring(tension, friction),
}
